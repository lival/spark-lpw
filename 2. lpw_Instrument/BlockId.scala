...
/*
@DeveloperApi
case class WholeBlockId(blockId: Int, isRdd: Boolean) extends BlockId {
  // blockId is the corresponding rdd's id
  override def name: String = "whole_" + blockId
  def RddBlock: Boolean = isRdd
  var weight: Double = 0
  val partitionWeight = new mutable.HashMap[Int, Double]
  val partitions: ArrayBuffer[BlockId] = new ArrayBuffer[BlockId]()
  def getweight(): Double = {
    return weight
  }
}
