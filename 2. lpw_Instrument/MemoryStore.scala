...
private[spark] class MemoryStore(
  ...
  private var refCount = new mutable.HashMap[Int, Int]()// key = rdd id, value = ref count
  ...
  def updateRef(jobId: Int, partitionCount: Int, refCountByJob: mutable.HashMap[Int, Int]): Unit = {
    ...
    //  logInfo("received ref count")
    //  logInfo(refCount.toString())
    //  logInfo("ref count end")
  ...
  def updateCost(partitionId: Int, map: mutable.HashMap[Int, Long]): Unit = {
    ...
    for (item <- iterator) {
      ...
      // val weight = calculateWeight(partitionId, item._1)
      calculateWeight(partitionId, item._1)
      /*
      weights.synchronized {
        val oldRDD = weights.getOrElse[WholeBlockId](item._1.toString, WholeBlockId(item._1, true))
        val oldPartitionWeight = oldRDD.partitionWeight.getOrElse(item._1, 0.toDouble)
        oldRDD.weight += (weight - oldPartitionWeight)
        weights.update(item._1.toString, oldRDD)
      }
       */
    }
  // logInfo("weight calculation cost is " + (System.currentTimeMillis() - curTime) + " millisecond")
  }

  def calculateWeight(partitionId: Int, rddid: Int): Unit = {// I decided to update weight here
    // instead of doing so in another function
    val blockId = keyToBlockId.get(rddid.toString + "_" + partitionId.toString)
    blockId match {
      case None =>
   //     logInfo("block not found while calculating weight, weight is zero")
      case Some(x) =>
        ...
        val cost = computationCost.synchronized {
          computationCost.get((rddid, partitionId)) match {
            case Some(x) =>
              x.toDouble
            case None =>
         //     logInfo("cost not found for rdd " + rddid)
              0.0
          }
        }
        val ref: Double = refCount.synchronized {
          refCount.get(rddid) match {
            case Some(x) =>
              x.toDouble
            case None =>
      //        logInfo("ref count not found for rdd " + rddid)
              0.0
          }
        }
      ...
      //  logInfo("weight for block " + rddid.toString + "_" + partitionId.toString + " is: " + weight)
      //  logInfo("size is: " + size + ", inMem is: " + inMem + ", cost is: " + cost + ", ref is: " + ref + ", pastmod is: " + pastmod)
  ...
  //    logInfo("block " + id.split("_")(1) + "_" + id.split("_")(2) + " put into local map")
    }
  }
  /*
  def putOrUpdateWholeBlock(id: String, partition: BlockId): Unit = {
    weights.synchronized {
      if (id.split("_")(0) != "rdd") {
        return
      }
      keyToBlockId.put(id.split("_")(1) + "_" + id.split("_")(2), partition)
      val rddid = id.split("_")(1)
      var realWeight = weights.get(rddid) match {
        case Some(x) =>
          x.weight
        case None =>
          0
      ...
      if (!weights.contains(rddid)) {
        val newRdd = WholeBlockId(rddid.toInt, partition.isRDD)
        newRdd.weight = realWeight
        newRdd.partitions += partition
        newRdd.partitionWeight.put(id.split("_")(2).toInt, realWeight)
        weights.put(rddid, newRdd)
      }
      else {
        val curRdd = weights.getOrElse[WholeBlockId
        ](rddid, WholeBlockId(rddid.toInt, partition.isRDD))
        curRdd.weight += realWeight
        curRdd.partitions += partition
        curRdd.partitionWeight.put(id.split("_")(2).toInt, realWeight)
        weights.update(rddid, curRdd)
      }
    }
  }
   */
  ...
  def putBytes[T: ClassTag](
    ...
    if (memoryManager.acquireStorageMemory(blockId, size, memoryMode)) {
      ...
      entries.synchronized {
        ...
        /*
        // instrument code
        putOrUpdateWholeBlock(blockId.name, blockId)
        // instrument code end
         */
      }
  ...
  def remove(blockId: BlockId): Boolean = memoryManager.synchronized {
    ...
    if (entry != null) {
      ...
      if (blockId.name.split("_")(0) == "rdd") {
        weights.synchronized {
          // instrument code
          val curblk = weights.getOrElse(blockId.name.split("_")(1), new WholeBlockId(-1, false))
          curblk.partitions -= blockId
          if (curblk.partitions.size > 0) {
            weights.update(blockId.name.split("_")(1), curblk)
          }
          else {
            weights.remove(blockId.name.split("_")(1))
          }
          keyToBlockId.remove(blockId.name.split("_")(1) + "_" + blockId.name.split("_")(2))
          // instrument code end
        }
  ...
  private[spark] def evictBlocksToFreeSpace(
    ...
    memoryManager.synchronized {
      ...
      entries.synchronized {
        ...
        val iterator = keyToBlockId.toList.sortBy(_._2.weight).iterator// modified
        while (freedMemory < space && iterator.hasNext) {
          ...
          val list: ArrayBuffer[BlockId] = pair._2.partitions
          for (bId <- list) {
            val entry = entries.get(bId)
            if (blockIsEvictable(bId, entry)) {
              ...
              if (blockInfoManager.lockForWriting(bId, blocking = false).isDefined) {
                ...
                logInfo("weight for " + bId + " is " + pair._2.weight)
              }
            }
          }
           */
          ...
          if (blockIsEvictable(blockId, entry)) {
            if (blockInfoManager.lockForWriting(blockId, blocking = false).isDefined) {
              ...
              freedMemory += entry.size
        //      logInfo("weight for " + blockId + " is " + pair._2.weight)
            }
          }
          /* if (blockIsEvictable(blockId, entry)) {
            // We don't want to evict blocks which are currently being read, so we need to obtain
